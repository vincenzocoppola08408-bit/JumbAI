      ...
      <aside className="fixed left-0 top-0 bottom-0 w-64 z-50 bg-surface border-r border-white/[0.06] flex flex-col">
        ...
        <nav className="flex-1 px-3 py-6 space-y-1">
          ...
          <div className="pt-4 pb-2">
            <p className="px-4 text-[10px] font-semibold uppercase tracking-wider text-coolGray/50">Categorie</p>
          </div>
          {CATEGORIE.slice(1).map((cat) => (
            <button
              key={cat.id}
              onClick={() => { setCategoria(cat.id); if (cat.prompt) setPrompt(cat.prompt); else setPrompt(''); setSezione('casa'); }}
              className={`sidebar-item w-full text-xs ${categoria === cat.id ? 'active' : ''}`}
            >
              <span>{cat.icona}</span>
              <span>{cat.id}</span>
            </button>
        ...
        <div className="glass-card rounded-3xl p-8 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-coolGray mt-1">
              {isRegistrazione ? 'Registrati per sbloccare il piano Premium' : 'Accedi per usare i tuoi crediti'}
            </p>
          </div>
          <div className="space-y-4">
            <div>
            ...
            <p className="text-center text-sm text-coolGray">
              {isRegistrazione ? 'Hai gi&agrave; un account?' : 'Non hai un account?'}{' '}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ink">
        <div className="glass-card rounded-3xl p-10 max-w-md text-center shadow-2xl border-violet/20">
          ...
          <p className="text-coolGray mb-6">Autenticati per accedere al generatore di immagini AI.</p>
          <button onClick={handleGoogleLogin} className="rounded-xl bg-gradient-to-r from-violet to-roseSoft text-white font-bold px-8 py-3 shadow-lg shadow-violet/30 hover:shadow-violet/50 transition">Continua con Google</button>
